...
Blockly.CSharp['controls_repeat_ext'] = function(block) {
  ...
  var code = '';
  var loopVar = Blockly.CSharp.nameDB_.getDistinctName(
...
Blockly.CSharp['controls_forEach'] = function(block) {
  ...
  if (!argument0.match(/^\w+$/)) {
    listVar = Blockly.CSharp.nameDB_.getDistinctName(
        variable0 + '_list', Blockly.VARIABLE_CATEGORY_NAME);
    code += 'var ' + listVar + ' = ' + argument0 + ';\n';
  }
  var indexVar = Blockly.CSharp.nameDB_.getDistinctName(
...
Blockly.CSharp['controls_flow_statements'] = function(block) {
  ...
  if (Blockly.CSharp.STATEMENT_PREFIX) {
    ...
    xfix += Blockly.CSharp.injectId(Blockly.CSharp.STATEMENT_PREFIX,
        block);
  }
  if (Blockly.CSharp.STATEMENT_SUFFIX) {
    // Inject any statement suffix here since the regular one at the end
    // will not get executed if the break/continue is triggered.
    xfix += Blockly.CSharp.injectId(Blockly.CSharp.STATEMENT_SUFFIX,
        block);
  }
  if (Blockly.CSharp.STATEMENT_PREFIX) {
